          ...
          <div className="checkout-main-column">
            ...
            <section className="checkout-panel">
              ...
              <div className="checkout-list">{items.map((item) => {
                ...
                return <article key={item.cart_item_id} className="checkout-item"><img className="checkout-thumb" src={image} alt={item.name} onError={(event) => { event.currentTarget.src = fallback; }} /><div className="checkout-item-copy"><strong>{item.name}</strong>{item.selected_color && <span className="cart-item-color"><b>Cor:</b> {item.selected_color}</span>}<span>R$ {Number(item.price).toFixed(2)} por unidade</span><small>Subtotal do item: R$ {subtotal.toFixed(2)}</small></div><div className="checkout-item-controls"><label>Quantidade<input type="number" min="1" value={item.quantity} onChange={(e) => upsertItem(item.product_id, Number(e.target.value), item.selected_color)} /></label><button className="btn btn-secondary" onClick={() => removeItem(item.cart_item_id)}>Remover</button></div></article>;
              })}</div>
            </section>
          </div>
